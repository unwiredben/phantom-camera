    ...
    switch (tuple->key) {
        case NETDL_DATA: {
            if (ctx->index + tuple->length <= ctx->length) {
                ...
                APP_LOG(APP_LOG_LEVEL_WARNING, "Not overriding rx buffer. Bufsize=%" PRId32 " BufIndex=%" PRId32 " DataLen=%u",
        ...
        case NETDL_END: {
            if (ctx->data && ctx->length > 0 && ctx->index > 0) {
                ...
                printf("Received file of size=%" PRIo32 " and address=%p", ctx->length, ctx->data);
                ctx->callback(image);
